...
once. #link(calepin.url("/palimpsest/"))[palimpsest] (manuscript revision letters)
and #link(calepin.url("/checkitoff/"))[checkitoff] (reporting-guideline checklists) are
...
that resolves the second one, are documented package-agnostically, with
a full worked example, at
...
- Manuscript revisions and a reviewer response letter that cites the
  real pages: #link(calepin.url("/palimpsest/"))[palimpsest]'s own manual.
- Reporting-guideline checklists (CONSORT, PRISMA, SPIRIT, STARD,
  STROBE) that cite the real pages: #link(calepin.url("/checkitoff/"))[checkitoff]'s own
  manual.
- A word-count and inventory audit of the composed manuscript, needing
  no per-passage markup at all: #link(calepin.url("/colophon/"))[colophon]'s own
  manual.
- Combining several such packages in one compile, with a full worked
  example and an explicit list of what works and what doesn't:
  #link(calepin.url("/combining/"))[Combining packages].
