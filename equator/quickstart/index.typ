...
#metadata((title: "Your first checklist", translation_key: "equator-quickstart")) <website-metadata>
...
compile, and #link("/equator/project/")[Wiring a real project] explains
...
#m.note(title: "About the examples in this manual")[
  ...
  #link("/equator/project/")[Wiring a real project].
]
...
instead (see #link("/equator/checklists/")[Built-in checklists]), but a
tiny one keeps the whole grid on one page here — a short manuscript,
and two `check()` calls plus one `na()`.

#m.snippet("/packages/equator/docs/manual-snippets/bundle-basics.typ")
...
+ `tiny` is nothing but a dictionary: a `name`, a `full-name`, and a
  ...
  an `id`, and a `description`. Nothing here is specific to equator's
...
+ `#show: contexture.bundle.with(...)` is the one line of setup a real
  ...
  as you'd write it without equator at all.
...
#m.screenshot("/packages/equator/docs/manual-snippets/bundle-basics/manuscript-plain.png")
...
#m.screenshot("/packages/equator/docs/manual-snippets/bundle-basics/manuscript-preview.png")

And here is `checklist.pdf`, from the first, plain compile — the actual
deliverable:

#m.screenshot("/packages/equator/docs/manual-snippets/bundle-basics/checklist-plain.png")
...
#link("/equator/diagnostics/")[Reading the grid], once the individual
...
+ Pick a checklist — one of the built-in ones
  (#link("/equator/checklists/")[Built-in checklists]), or your own
...
+ Once the draft feels complete, compile normally and read
  ...
  (#link("/equator/diagnostics/")[below]) lists anything unresolved: an
...
+ Before submission, compile once with `strict: true`
  (#link("/equator/diagnostics/#strict-mode")[below]): every remaining
...
#m.chapter-nav(
  prev: ("/equator/", "equator overview"),
  next: ("/equator/marking/", "Marking items: check and na"),
)
